  ...
  <head>
    ...
    <style>${styles}
      ...
      * { box-sizing: border-box; }
      ...
      .export-body .split.split--bottom { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  ...
  return `<details class="export-fight"${open}>
    ...
    <div class="export-fight__body">
      ...
      <section class="split split--bottom">
        ${renderExportEventPanel("Fire mage watch", "Ignite warnings", renderExportIgniteRows(fight))}
...
function renderExportIgniteRows(fight) {
  return fight.igniteWarnings.map((warning) => `<article class="event ${warning.likelyPushedOff ? "event--bad" : ""}">
    <div class="event__top"><span class="event__title">${escapeHtml(warning.target || "Unknown target")}</span><strong>${warning.age.toFixed(1)}s</strong></div>
  ...
  </article>`).join("") || `<article class="event muted">No Ignite warnings found.</article>`;
}
...
function getBossPressureRows(fight) {
  const rows = (fight.type === "boss" ? fight.bossDebuffs ?? fight.debuffs : fight.debuffs)
    .filter((item) => item.spell !== "Ignite" && item.seconds >= 1);
