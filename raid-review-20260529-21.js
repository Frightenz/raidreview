...
function analyzeFight(segment, index, warriors, allPlayers = [], classPlayers = {}) {
  ...
  return {
    ...
    bossActiveStart,
    bossActiveDuration,
    trinkets: summarizeTrinkets(trinketIntervals, trinketUses, duration, segment.name, start),
    ...
    lowActivity: summarizeLowActivity(allPlayers, summarizeActivityWindows(activityWindowsByPlayer, start, end), duration),
    igniteWarnings,
    deaths
  };
}
...
function summarizeActivityWindows(store, fightStart, fightEnd) {
...
function summarizeTrinkets(store, useStore, fightDuration, fightName, fightStart = 0) {
  return [
    ...summarizeIntervals(store, fightDuration, true, fightStart)
    .map((row) => {
    ...
    ...summarizeConsumableUses(useStore, fightDuration, fightStart)
  ]
...
function summarizeConsumableUses(store, fightDuration, fightStart = 0) {
  return [...store.values()].map((record) => {
    ...
    const note = [
      `${record.uses} use${record.uses === 1 ? "" : "s"}`,
  ...
  return `<details class="export-fight"${open}>
    <summary>
      <div class="export-fight__head">
        ...
        <span class="pill">${escapeHtml(formatDuration(fight.duration))}</span>
      </div>
    </summary>
