...
function parseLine(line) {
  ...
  if (event.startsWith("SWING")) {
    ...
    if (event === "SPELL_ENERGIZE") base.wastedEnergy = numberOrNull(fields[31]) ?? 0;
...
function addConsumableUse(store, event) {
  ...
  const record = store.get(key) ?? { spell: event.spellName, target, targetGuid, uses: 0, times: [], wastedEnergy: 0, energyGained: 0 };
...
function summarizeConsumableUses(store, fightDuration, fightStart = 0, activeWindows = [], fightType = "boss") {
  return [...store.values()].map((record) => {
    ...
    const flagWastedEnergy = shouldFlagWastedEnergy(record.spell, wastedEnergy);
    ...
    const note = [
      ...
      wastedEnergy > 0 ? `${wastedEnergy} energy wasted` : ""
    ].filter(Boolean).join("; ");
    return {
      spell: record.spell,
      target: record.target,
      ...
      wastedEnergy,
      note,
      warning: flagWastedEnergy ? `${wastedEnergy} energy wasted.` : "",
      hideWarningText: flagWastedEnergy
    };
  }).filter((row) => row.uses > 0);
}

function shouldFlagWastedEnergy(spell, wastedEnergy) {
